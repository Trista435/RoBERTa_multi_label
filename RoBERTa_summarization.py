...
from transformers import (
    ...
    Trainer, TrainerCallback, EvalPrediction, AutoTokenizer, AutoModelForSeq2SeqLM, PegasusTokenizer, PegasusForConditionalGeneration)
...
# ===== summarization 模型 =====
model_name = "chiakya/Bert-chinese-Summarization"
tokenizer = AutoTokenizer.from_pretrained(model_name)
model = AutoModelForSeq2SeqLM.from_pretrained(model_name).to(device)

# ===== 定義摘要函數 =====
def summarize_text(text, max_input_length=512, max_output_length=60):
...
# ===== 批次摘要處理 =====
tqdm.pandas(desc="摘要處理中")
df['summary'] = df['text'].progress_apply(summarize_text)

# ===== 輸出確定結果 =====
summary_map = df.set_index('text')['summary'].to_dict()
grouped['summary'] = grouped['text'].map(summary_map)


# ===== 儲存摘要 =====
train_texts.to_csv("train_texts_summarized.csv", index=False, encoding='utf-8-sig')
val_texts.to_csv("val_texts_summarized.csv", index=False, encoding='utf-8-sig')
X_test.to_csv("test_texts_summarized.csv", index=False, encoding='utf-8-sig')
...
output_dir = "./model_regularization_summarization"
...
# ===== 儲存驗證集預測 =====
predict_and_save(val_texts, val_labels, "val_predictions_regularization_summarization.csv")

# ===== 儲存測試集預測 =====
predict_and_save(X_test, y_test, "test_predictions_regularization_summarization.csv")
...
plt.legend()
plt.grid(True)
plt.savefig("loss_curve_regularization_summarization.png")