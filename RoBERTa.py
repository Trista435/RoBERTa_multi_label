...
##def map_to_unit(text):
  #  if text in label_mapping:
  #      return label_mapping[text]
  # # for keyword, unit in fuzzy_mapping.items():
   #     if keyword in text:
   #         return unit
   #         return 未知
...
def compute_metrics(pred: EvalPrediction):
    ...
    preds_binary = (preds >= 0.8).astype(int)                               #threshold
...
def predict_and_save(texts, labels, filename, batch_size=32):
    ...
    preds_binary = (probs >= 0.8).astype(int)                            # threshold
