...
# ===== 多標籤編碼（僅用於 test）=====
grouped_multi_all = df.groupby('text')['label'].apply(list).reset_index()
grouped_multi_dict = dict(zip(grouped_multi_all['text'], grouped_multi_all['label']))

# 確保 y_test_multi 的順序與 X_test 完全一致
X_test_original = grouped['text'].iloc[X_test.index]  # 這是原始未分詞的順序

# 根據 text 去找回對應的多標籤（確保順序）
y_test_multi_labels = X_test_original.map(grouped_multi_dict).tolist()

# 若某些 text 落單沒出現在 grouped_multi_dict，會是 NaN → 轉空列表
y_test_multi_labels = [labels if isinstance(labels, list) else [] for labels in y_test_multi_labels]

mlb = MultiLabelBinarizer(classes=label_encoder.classes_)
y_test_multi = mlb.fit_transform(y_test_multi_labels)
...
# 多標籤：y_test_multi
pd.DataFrame(y_test_multi, columns=mlb.classes_).to_csv("test_labels_multilabel_jieba.csv", index=False, encoding='utf-8-sig')

# ===== 儲存編碼器 =====
joblib.dump(label_encoder, "label_encoder.pkl")
joblib.dump(mlb, "multilabel_binarizer.pkl")

print("✅ 前處理完成，單標籤與多標籤 test 資料皆已儲存。")
